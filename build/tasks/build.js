...
module.exports = function( grunt ) {
	...
	function convert( name, path, contents ) {
		...
		// AMD Name
			...
			contents = contents
				.replace( /(\s*)"AMDNAME"(\,\s*)/, amdName ? "$1\"" + amdName + "\"$2" : "" )
		...
		*/
			...
			// Ignore mainName's exports (the only necessary one)
			if ( name !== mainName ) {
				contents = contents
			...
			// AMD Name
			if ( /^exports\/amd$/.test( name ) && (amdName = grunt.option( "amd" )) == null ) {
				contents = contents
					.replace( /(\s*)"AMDNAME"(\,\s*)/, mainName ? "$1\"" + mainName + "\"$2" : "" );
			}
	...
	function() {
		...
		if ( !optIn ) {
			...
			config.rawText[mainName] = "define([" + (included.length ? included.join(",") : "") + "]);";
		}
